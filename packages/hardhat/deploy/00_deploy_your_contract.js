...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  */
  ...
  const triageGroup = await deploy("TriageGroup", {
    from: deployer,
    log: true,
  })
  const poh = await deploy("POHmimic", {
    from: deployer,
    log: true,
  })
  ...
  const grantManager = await deploy("GrantManager", {
    // Learn more about args here: https://www.npmjs.com/package/hardhat-deploy#deploymentsdeploy
    from: deployer,
    args: [gtc.address, poh.address, triageGroup.address],
    log: true,
  });
